...
/**
 * @brief  Convert 32-bit host byte order to network byte order (big-endian)
 * @param  host32  32-bit unsigned integer in host byte order
 * @return        32-bit unsigned integer in network byte order (big-endian)
 */
 ...
 * @param  net32   32-bit unsigned integer in network byte order (big-endian)
...
/**
 * @brief  Application-layer protocol message type enum
 * @note   Values transmitted in network byte order (big-endian) on wire.\n
 *         Range: 0x0001-0x0FFF base protocol, 0x1000+ extended
 */
 ...
 * @param  json  UTF-8 JSON message body (excluding 8-byte header)
 ...
 * @param  header   Pointer to 8-byte binary header
 * @param  outType  [out] Parsed message type (network-to-host order)
 * @param  outLength [out] Parsed message body length (bytes)
 * @return 成功返回 true
 */
...
class RecvBuffer {
public:
    /** @brief Append received bytes to internal buffer
    ...
    ///< Buffer current pending byte count
    size_t pending() const { return m_buf.size(); }
    ///< Clear buffer (call after abnormal disconnect)
...
public:
    /** @brief Hash functor for MsgType used by unordered_map
     * @note  Required because MsgType is enum class, not plain int
     */
    struct MsgTypeHash {
     ...
     * @param  handler Handler function (lambda/std::bind/function pointer)
     */
    void on(MsgType type, Handler handler) {
        m_handlers[type] = std::move(handler);
    }

    /** @brief Dispatch message to registered handler
     * @param  type  Message type
     * @param  json  UTF-8 JSON message body
     * @return true if handler found and called; false if no handler registered
     */
...
private:
    std::unordered_map<MsgType, Handler, MsgTypeHash> m_handlers;  ///< type-to-handler map
};
